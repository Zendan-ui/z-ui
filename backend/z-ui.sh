...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/Zendan-ui/z-ui/main/scripts/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
custom_version() {
    ...
    download_link="https://raw.githubusercontent.com/Zendan-ui/z-ui/main/scripts/install.sh"
...
uninstall() {
    ...
    echo ""
    echo -e "Uninstalled Successfully, If you want to remove this script, then after exiting the script run ${green}rm /usr/local/z-ui -f${plain} to delete it."
    echo ""
...
reset_admin() {
    ...
    if [[ $? == 0 ]]; then
        /usr/local/z-ui/z-ui admin -reset
    fi
    before_show_menu
}
...
set_admin() {
    ...
    /usr/local/z-ui/z-ui admin -username ${config_account} -password ${config_password}
    before_show_menu
}

view_admin() {
    /usr/local/z-ui/z-ui admin -show
    before_show_menu
}
...
set_setting() {
    ...
    /usr/local/z-ui/z-ui setting ${params}
    before_show_menu
}

view_setting() {
    /usr/local/z-ui/z-ui setting -show
    view_uri
    before_show_menu
}

view_uri() {
    info=$(/usr/local/z-ui/z-ui uri)
    if [[ $? != 0 ]]; then
...
update_shell() {
    wget -O /usr/bin/z-ui -N --no-check-certificate https://github.com/Zendan-ui/z-ui/raw/main/backend/z-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
...
check_z_ui_status() {
    count=$(ps -ef | grep "z-ui" | grep -v "grep" | wc -l)
    if [[ count -ne 0 ]]; then
        return 0
    else
        return 1
...
show_z_ui_status() {
    check_z_ui_status
    if [[ $? == 0 ]]; then
        echo -e "z-ui state: ${green}Running${plain}"
    else
        echo -e "z-ui state: ${red}Not Running${plain}"
    fi
}
...
show_usage() {
    echo -e "Z-UI Control Menu Usage"
    echo -e "------------------------------------------"
    echo -e "SUBCOMMANDS:" 
    ...
    echo -e "z-ui status       - Current Status of z-ui"
    ...
    echo -e "z-ui help         - Control Menu Usage"
    echo -e "------------------------------------------"
}

show_menu() {
  echo -e "
  ${green}Z-UI Admin Management Script ${plain}
————————————————————————————————
  ${green}0.${plain} Exit
————————————————————————————————
...
————————————————————————————————
  ${green}11.${plain} Z-UI Start
  ${green}12.${plain} Z-UI Stop
  ${green}13.${plain} Z-UI Restart
  ${green}14.${plain} Z-UI Check State
  ${green}15.${plain} Z-UI Check Logs
  ${green}16.${plain} Z-UI Enable Autostart
  ${green}17.${plain} Z-UI Disable Autostart
————————————————————————————————
...
————————————————————————————————
 "
    ...
    10)
        ...
        ;;
    11)
        check_install && start z-ui
        ;;
    12)
        check_install && stop z-ui
        ;;
    13)
        check_install && restart z-ui
        ;;
    14)
        check_install && status z-ui
        ;;
    15)
        check_install && show_log z-ui
        ;;
    16)
        check_install && enable z-ui
        ;;
    17)
        check_install && disable z-ui
        ;;
    18)
        bbr_menu
        ;;
...
if [[ $# > 0 ]]; then
    case $1 in
    "start")
        check_install 0 && start z-ui 0
        ;;
    "stop")
        check_install 0 && stop z-ui 0
        ;;
    "restart")
        check_install 0 && restart z-ui 0
        ;;
    "status")
        check_install 0 && status 0
        ;;
    "enable")
        check_install 0 && enable z-ui 0
        ;;
    "disable")
        check_install 0 && disable z-ui 0
        ;;
    "log")
        check_install 0 && show_log z-ui 0
        ;;
    "update")
