...
> tree rather than a pinned release (Go module `github.com/mhsanaei/z-ui/v3`).
>
...
z-ui is a **web control panel for [Xray-core](https://github.com/XTLS/Xray-core)**. The Go
...
2. **The Runtime abstraction (multi-node).** A panel can manage remote "nodes" (other z-ui
  ...
  → POST {basePath}/panel/api/...
    → Gin engine (internal/web/web.go: initRouter)
      → middleware chain: SecurityHeaders → MaxBodyBytes (10 MiB; importDB exempt)
                          → [DomainValidator, if webDomain set] → gzip → sessions("z-ui")
...
├── install.sh / update.sh / z-ui.sh                        # VPS install + management CLI
├── z-ui.service.*  / z-ui.rc                               # systemd units (debian/rhel/arch) + rc script
...
A "node" (`model.Node`) is another z-ui instance this panel controls. Every state-changing
...
- **Module path is `.../v3`.** Internal imports use `github.com/mhsanaei/z-ui/v3/internal/...`.
- **SQLite vs Postgres.** Default is SQLite at `{XUI_DB_FOLDER}/z-ui.db`. Postgres via
