...
log_folder="${XUI_LOG_FOLDER:=/var/log/z-ui}"
mkdir -p "${log_folder}"
...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/MHSanaei/3z-ui/main/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update() {
    confirm "This function will update all z-ui components to the latest version, and the data will not be lost. Do you want to continue?" "y"
    if [[ $? != 0 ]]; then
        LOGE "Cancelled"
        if [[ $# == 0 ]]; then
            before_show_menu
    ...
    bash <(curl -Ls https://raw.githubusercontent.com/MHSanaei/3z-ui/main/update.sh)
    if [[ $? == 0 ]]; then
...
update_dev() {
    confirm "This will update z-ui to the latest DEV commit (the rolling 'dev-latest' build, not a stable release). Your data is preserved. Continue?" "y"
    if [[ $? != 0 ]]; then
        LOGE "Cancelled"
        if [[ $# == 0 ]]; then
            before_show_menu
    ...
    XUI_UPDATE_TAG="dev-latest" bash <(curl -Ls https://raw.githubusercontent.com/MHSanaei/3z-ui/main/update.sh)
    if [[ $? == 0 ]]; then
...
replace_xui_script() {
    ...
    local temp_file="/usr/bin/z-ui-temp.$$"

    rm -f "$temp_file"
    if [[ "$use_if_modified_since" == "true" ]]; then
        curl -fLRo "$temp_file" -z /usr/bin/z-ui "$url"
    else
...
update_menu() {
    ...
    if replace_xui_script "https://raw.githubusercontent.com/MHSanaei/3z-ui/main/z-ui.sh" "false"; then
        chmod +x ${xui_folder}/z-ui.sh
...
legacy_version() {
    ...
    install_command="bash <(curl -Ls "https://raw.githubusercontent.com/mhsanaei/3z-ui/v$tag_version/install.sh") v$tag_version"
...
uninstall() {
    ...
    if [[ $release == "alpine" ]]; then
        ...
        rc-update del z-ui
        rm /etc/init.d/z-ui -f
    else
        systemctl stop z-ui
        systemctl disable z-ui
        rm ${xui_service}/z-ui.service -f
    ...
    echo -e "${green}bash <(curl -Ls https://raw.githubusercontent.com/mhsanaei/3z-ui/master/install.sh)${plain}"
    echo ""
...
reset_user() {
    ...
    if [[ $twoFactorConfirm != "y" && $twoFactorConfirm != "Y" ]]; then
        ${xui_folder}/z-ui setting -username "${config_account}" -password "${config_password}" > /dev/null 2>&1
    else
        ${xui_folder}/z-ui setting -username "${config_account}" -password "${config_password}" -resetTwoFactor=true > /dev/null 2>&1
...
reset_webbasepath() {
    ...
    ${xui_folder}/z-ui setting -webBasePath "${config_webBasePath}" > /dev/null 2>&1
...
reset_config() {
    ...
    ${xui_folder}/z-ui setting -reset
    echo -e "All panel settings have been reset to default."
    restart
}

check_config() {
    local info=$(${xui_folder}/z-ui setting -show true)
    if [[ $? != 0 ]]; then
    ...
    if [[ -r "$db_env_file" ]] && grep -q '^XUI_DB_TYPE=postgres' "$db_env_file"; then
        ...
        echo -e "${green}Database: SQLite (/etc/z-ui/z-ui.db)${plain}"
    fi
...
set_port() {
    ...
    if [[ -z "${port}" ]]; then
        ...
        ${xui_folder}/z-ui setting -port ${port}
...
restart_xray() {
    ...
    if [[ $release == "alpine" ]]; then
        rc-service z-ui reload
    else
        systemctl reload z-ui
    fi
    LOGI "xray-core Restart signal sent successfully, Please check the log information to confirm whether xray restarted successfully"
    sleep 2
    show_xray_status
    ...
    if [[ $release == "alpine" ]]; then
        ...
        case "$choice" in
            0)
                ...
                ;;
            1)
                journalctl -u z-ui -e --no-pager -f -p debug
                if [[ $# == 0 ]]; then
                    before_show_menu
                fi
                ;;
...
disable_bbr() {
    ...
    if [ -f "/etc/sysctl.d/99-bbr-z-ui.conf" ]; then
        old_settings=$(head -1 /etc/sysctl.d/99-bbr-z-ui.conf | tr -d '#')
        ...
        rm /etc/sysctl.d/99-bbr-z-ui.conf
    else
...
enable_bbr() {
    ...
    if [ -d "/etc/sysctl.d/" ]; then
        ...
        sysctl -p /etc/sysctl.d/99-bbr-z-ui.conf
    else
...
update_shell() {
    if replace_xui_script "https://github.com/MHSanaei/3z-ui/raw/main/z-ui.sh" "true"; then
...
xui_pid() {
    ps -ef 2> /dev/null | grep -F "${xui_folder}/z-ui" | grep -v grep | awk 'NR==1 {print $1}'
}
...
check_status() {
    ...
    if [[ $release == "alpine" ]]; then
        ...
        if [[ $(rc-service z-ui status | grep -F 'status: started' -c) == 1 ]]; then
            return 0
        else
            return 1
        fi
...
check_enabled() {
    if [[ $release == "alpine" ]]; then
        if [[ $(rc-update show | grep -F 'z-ui' | grep default -c) == 1 ]]; then
            return 0
        else
            return 1
        fi
...
ssl_cert_issue_main() {
    ...
    case "$choice" in
        ...
        4)
            ...
            local panel_cert=$(${xui_folder}/z-ui setting -getCert true | grep 'cert:' | awk -F': ' '{print $2}' | tr -d '[:space:]')
                    ...
                    if [[ -f "${webCertFile}" && -f "${webKeyFile}" ]]; then
                        ...
                        if command -v ~/.acme.sh/acme.sh &> /dev/null && ~/.acme.sh/acme.sh --list 2> /dev/null | awk '{print $1}' | grep -Fxq "${domain}"; then
                            ~/.acme.sh/acme.sh --installcert --force -d "${domain}" \
                                ...
                                --reloadcmd "z-ui restart" 2>&1 || true
...
ssl_cert_issue_for_ip() {
    ...
    local existing_webBasePath=$(${xui_folder}/z-ui setting -show true | grep -Eo 'webBasePath: .+' | awk '{print $2}')
    ...
    local reloadCmd="systemctl restart z-ui 2>/dev/null || rc-service z-ui restart 2>/dev/null"
...
ssl_cert_issue() {
    ...
    reloadCmd="z-ui restart"

    LOGI "Default --reloadcmd for ACME is: ${yellow}z-ui restart"
    LOGI "This command will run on every certificate issue and renew."
    read -rp "Would you like to modify --reloadcmd for ACME? (y/n): " setReloadcmd
    if [[ "$setReloadcmd" == "y" || "$setReloadcmd" == "Y" ]]; then
        echo -e "\n${green}\t1.${plain} Preset: systemctl reload nginx ; z-ui restart"
        echo -e "${green}\t2.${plain} Input your own command"
        echo -e "${green}\t0.${plain} Keep default reloadcmd"
        read -rp "Choose an option: " choice
        case "$choice" in
            1)
                LOGI "Reloadcmd is: systemctl reload nginx ; z-ui restart"
                reloadCmd="systemctl reload nginx ; z-ui restart"
                ;;
            2)
                LOGD "It's recommended to put z-ui restart at the end, so it won't raise an error if other services fails"
                read -rp "Please enter your reloadcmd (example: systemctl reload nginx ; z-ui restart): " reloadCmd
...
# setup_fail2ban_iplimit directly via `z-ui setup-fail2ban`.
install_iplimit() {
    setup_fail2ban_iplimit
    before_show_menu
}
...
EOF
    ...
    local panel_port
    panel_port=$(${xui_folder}/z-ui setting -show true 2>/dev/null | grep -Eo 'port: .+' | awk '{print $2}')
...
SSH_port_forwarding() {
    ...
    local existing_webBasePath=$(${xui_folder}/z-ui setting -show true | grep -Eo 'webBasePath: .+' | awk '{print $2}')
    local existing_port=$(${xui_folder}/z-ui setting -show true | grep -Eo 'port: .+' | awk '{print $2}')
    local existing_listenIP=$(${xui_folder}/z-ui setting -getListen true | grep -Eo 'listenIP: .+' | awk '{print $2}')
    local existing_cert=$(${xui_folder}/z-ui setting -getCert true | grep -Eo 'cert: .+' | awk '{print $2}')
    local existing_key=$(${xui_folder}/z-ui setting -getCert true | grep -Eo 'key: .+' | awk '{print $2}')
    ...
    case "$num" in
        1)
            if [[ -z "$existing_listenIP" || "$existing_listenIP" == "0.0.0.0" ]]; then
                ...
                ${xui_folder}/z-ui setting -listenIP "${config_listenIP}" > /dev/null 2>&1
            ...
            ;;
        2)
            ${xui_folder}/z-ui setting -listenIP 0.0.0.0 > /dev/null 2>&1
...
# needs this:   z-ui pgclient <major>
pg_upgrade_client() {
    local want="$1" have
...
migrate_to_postgres() {
    ...
    if ! ${xui_folder}/z-ui migrate-db --dsn "$dsn"; then
...
# file using the bundled z-ui binary. With no arguments it dumps the installed
# panel database; an optional second argument overrides the output path.
#   z-ui migrateDB [file.db|file.dump] [output]
migrate_db() {
    local input="$1" output="$2"
    local default_db="/etc/z-ui/z-ui.db"
    local bin="${xui_folder}/z-ui"

    [[ -z "$input" ]] && input="$default_db"

    if [[ ! -x "$bin" ]]; then
        LOGE "z-ui binary not found at ${bin}. Is the panel installed?"
        return 1
    fi

    if ! "$bin" migrate-db -h 2>&1 | grep -q -- '-dump'; then
        LOGE "This z-ui build does not support .db <-> .dump conversion yet."
        LOGE "Update the panel first (z-ui update) to a version with 'migrate-db --dump/--restore'."
        return 1
    fi

    if [[ ! -f "$input" ]]; then
        LOGE "Input file not found: ${input}"
        echo -e "Usage: ${green}z-ui migrateDB [file.db|file.dump] [output]${plain}"
        return 1
    fi
    ...
    if [[ "$mode" == "dump" ]]; then
        ...
        if [[ "$output" == "$default_db" ]] && check_status > /dev/null 2>&1; then
            LOGE "Refusing to restore into the live database (${default_db}) while z-ui is running."
            LOGE "Stop the panel first (z-ui stop) or choose a different output path."
            return 1
        fi
        if [[ -f "$output" ]]; then
            confirm "Output ${output} already exists and will be overwritten. Continue?" "n" || return 0
...
|  ${blue}z-ui restart-xray${plain}          - Restart Xray                     │
...
│  ${blue}z-ui update-dev${plain}            - Update to Dev channel (latest)   │
│  ${blue}z-ui update-all-geofiles${plain}   - Update all geo files             │
│  ${blue}z-ui migrateDB [file]${plain}      - Convert .db <-> .dump (SQLite)   │
│  ${blue}z-ui pgclient [ver]${plain}        - Upgrade pg_dump/pg_restore tools │
