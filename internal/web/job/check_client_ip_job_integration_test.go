...
// z-ui logger must be initialised once before any code path that can
...
func setupIntegrationDB(t *testing.T) {
	...
	t.Setenv("ZUI_DB_FOLDER", dbDir)
	t.Setenv("ZUI_LOG_FOLDER", logDir)
	...
	if err := database.InitDB(filepath.Join(dbDir, "z-ui.db")); err != nil {
...
func readIpLimitLogPath() string {
	folder := os.Getenv("ZUI_LOG_FOLDER")
	if folder == "" {
