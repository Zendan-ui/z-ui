...
const (
	panelUpdaterURL      = "https://raw.githubusercontent.com/MHSanaei/z-ui/main/update.sh"
...
// GetUpdateInfo checks GitHub for the latest z-ui release. When the dev channel
...
func (s *PanelService) startUpdate(useDev bool) (int64, error) {
	...
	runIDEnv := "ZUI_UPDATE_RUN_ID=" + strconv.FormatInt(runID, 10)
	statusFileEnv := "ZUI_UPDATE_STATUS_FILE=" + statusFile

	if systemdRun, err := exec.LookPath("systemd-run"); err == nil {
		unitName := fmt.Sprintf("z-ui-web-update-%d", time.Now().Unix())
		cmd := exec.CommandContext(context.Background(), systemdRun,
			"--unit", unitName,
			"--setenv", "ZUI_MAIN_FOLDER="+mainFolder,
			"--setenv", "ZUI_SERVICE="+serviceFolder,
			"--setenv", "ZUI_UPDATE_TAG="+updateTag,
			"--setenv", runIDEnv,
	...
	cmd.Env = append(os.Environ(),
		"ZUI_MAIN_FOLDER="+mainFolder,
		"ZUI_SERVICE="+serviceFolder,
		"ZUI_UPDATE_TAG="+updateTag,
		runIDEnv,
		statusFileEnv,
	)
...
func downloadPanelUpdater() (string, error) {
	...
	file, err := os.CreateTemp("", "z-ui-update-*.sh")
	if err != nil {
		return "", err
	}
	path := file.Name()
...
func fetchPanelRelease(tag string) (*service.Release, error) {
	url := "https://api.github.com/repos/MHSanaei/z-ui/releases/latest"
	if tag != "" {
		url = "https://api.github.com/repos/MHSanaei/z-ui/releases/tags/" + tag
	}
...
func resolveUpdateFolders() (string, string) {
	mainFolder := os.Getenv("ZUI_MAIN_FOLDER")
	if mainFolder == "" {
	...
	if mainFolder == "" {
		mainFolder = "/usr/local/z-ui"
	}

	serviceFolder := os.Getenv("ZUI_SERVICE")
	if serviceFolder == "" {
