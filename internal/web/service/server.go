...
func isFail2banEnabled() bool {
	value, ok := os.LookupEnv("ZUI_ENABLE_FAIL2BAN")
...
func (s *ServerService) GetLogs(count string, level string, syslog string) []string {
	...
	if syslog == "true" {
		...
		cmd := exec.CommandContext(context.Background(), "journalctl", "-u", "z-ui", "--no-pager", "-n", strconv.Itoa(countInt), "-p", level)
		var out bytes.Buffer
		cmd.Stdout = &out
		err = cmd.Run()
		if err != nil {
			return []string{"Failed to run journalctl command! Make sure systemd is available and z-ui service is registered."}
		}
...
// is .dump on PostgreSQL and .db on SQLite; the base falls back to "z-ui" when
// no address is known.
...
// notably the Telegram bot — still get a real address instead of the "z-ui"
// fallback.
...
// hyphen. Returns "z-ui" when nothing usable remains.
...
func (s *ServerService) GetMigration() ([]byte, string, error) {
	if database.IsPostgres() {
		tmp, err := os.CreateTemp("", "z-ui-migration-*.db")
		if err != nil {
			return nil, "", err
		}
		tmpPath := tmp.Name()
...
func pgRestoreReadFailureError(probeOutput, localVersion string) error {
	...
	if major, known := pgArchiveVersionIntroducedIn[m[1]]; known {
		return common.NewErrorf("This backup was created by pg_dump from PostgreSQL %d or newer, but the server's pg_restore is version %s and cannot read it; run 'z-ui pgclient %d' on the server (or upgrade the postgresql-client package to version %d or newer), then retry the import", major, localVersion, major, major)
	}
...
func (s *ServerService) importPostgresDB(file multipart.File) error {
	...
	tempFile, err := os.CreateTemp("", "z-ui-pg-restore-*.dump")
	if err != nil {
