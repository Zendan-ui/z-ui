...
import (
	...
	xuilogger "github.com/Zendan-ui/z-ui/v3/internal/logger"
...
// (ZUI_DB_TYPE=postgres + ZUI_DB_DSN) or SQLite (ZUI_SCALE_TEST=1, temp file;
// ZUI_SCALE_DB_PATH persists the DB for manual smoke runs), and registers
// cleanup. Skips the test when neither backend is configured.
func setupScaleDB(t *testing.T) {
	t.Helper()
	xuilogger.InitLogger(logging.ERROR)

	if os.Getenv("ZUI_DB_TYPE") == "postgres" && strings.TrimSpace(os.Getenv("ZUI_DB_DSN")) != "" {
	...
	switch strings.ToLower(strings.TrimSpace(os.Getenv("ZUI_SCALE_TEST"))) {
	case "1", "true", "yes":
		dbPath := strings.TrimSpace(os.Getenv("ZUI_SCALE_DB_PATH"))
		if dbPath == "" {
	...
	t.Skip("set ZUI_SCALE_TEST=1 (sqlite) or ZUI_DB_TYPE=postgres + ZUI_DB_DSN (postgres) to run the scale benchmark")
}

// scaleSizes returns the default size ladder unless ZUI_SCALE_SIZES overrides
// it with a comma-separated list (e.g. "500000" or "10000,100000,500000").
func scaleSizes(t *testing.T, def ...int) []int {
	t.Helper()
	raw := strings.TrimSpace(os.Getenv("ZUI_SCALE_SIZES"))
	if raw == "" {
		return def
	}
	var out []int
	for _, part := range strings.Split(raw, ",") {
		...
		if err != nil || n <= 0 {
			t.Fatalf("ZUI_SCALE_SIZES: invalid size %q", part)
		}
		out = append(out, n)
	}
	if len(out) == 0 {
