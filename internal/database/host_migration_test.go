...
func initMigrateDB(t *testing.T) {
	t.Helper()
	if err := InitDB(filepath.Join(t.TempDir(), "z-ui.db")); err != nil {
...
func TestMigrate_Postgres(t *testing.T) {
	if strings.TrimSpace(os.Getenv("ZUI_DB_DSN")) == "" || os.Getenv("ZUI_DB_TYPE") != "postgres" {
		t.Skip("set ZUI_DB_TYPE=postgres and ZUI_DB_DSN to run the postgres migration test")
	}
