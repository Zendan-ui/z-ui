...
// for the z-ui panel using GORM with SQLite or PostgreSQL.
package database
...
import (
	...
	"github.com/Zendan-ui/z-ui/v3/internal/database/model"
	"github.com/Zendan-ui/z-ui/v3/internal/util/crypto"
	"github.com/Zendan-ui/z-ui/v3/internal/util/random"
...
// enforced on this host: the integration must be enabled (ZUI_ENABLE_FAIL2BAN)
...
func fail2banCanEnforce() bool {
	if v, ok := os.LookupEnv("ZUI_ENABLE_FAIL2BAN"); ok && v != "true" {
		return false
	}
...
// When ZUI_DB_TYPE=postgres, dbPath is ignored and ZUI_DB_DSN is used instead.
func InitDB(dbPath string) error {
	...
	case "postgres":
		dsn := config.GetDBDSN()
		if dsn == "" {
			return errors.New("ZUI_DB_TYPE=postgres but ZUI_DB_DSN is empty")
		}
	...
	default:
		...
		pragmas := []string{
			...
			fmt.Sprintf("PRAGMA cache_size=-%d", envInt("ZUI_DB_CACHE_MB", 32)*1024),
			fmt.Sprintf("PRAGMA mmap_size=%d", int64(envInt("ZUI_DB_MMAP_MB", 256))*1024*1024),
	...
	switch config.GetDBKind() {
	case "postgres":
		maxOpen = envInt("ZUI_DB_MAX_OPEN_CONNS", 25)
		maxIdle = envInt("ZUI_DB_MAX_IDLE_CONNS", 25)
	default:
		maxOpen = envInt("ZUI_DB_MAX_OPEN_CONNS", 8)
		maxIdle = envInt("ZUI_DB_MAX_IDLE_CONNS", 4)
	}
...
func sqliteSynchronous() string {
	switch strings.ToUpper(strings.TrimSpace(os.Getenv("ZUI_DB_SYNCHRONOUS"))) {
	case "OFF":
		return "OFF"
	case "NORMAL":
		return "NORMAL"
