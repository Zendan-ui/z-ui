...
// during `z-ui migrate-db --dsn` (SQLite → PostgreSQL data migration) and in
// related tests.
//
...
func MigrateData(srcPath, dstDSN string) error {
	...
	log.Println("Set ZUI_DB_TYPE=postgres and ZUI_DB_DSN=... in /etc/default/z-ui, then restart z-ui.")
	return nil
}
