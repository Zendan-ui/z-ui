// Package config provides configuration management utilities for the z-ui panel,
...
// GetBaseVersion returns the raw embedded release version of the z-ui panel
...
// GetName returns the name of the z-ui application.
func GetName() string {
...
func GetLogLevel() LogLevel {
	...
	logLevel := os.Getenv("ZUI_LOG_LEVEL")
	if logLevel == "" {
		return Info
	}
...
// IsDebug returns true if debug mode is enabled via the ZUI_DEBUG environment variable.
func IsDebug() bool {
	return os.Getenv("ZUI_DEBUG") == "true"
}

// IsSkipHSTS returns true if skipping HSTS mode is enabled via the ZUI_SKIP_HSTS environment variable.
func IsSkipHSTS() bool {
	return os.Getenv("ZUI_SKIP_HSTS") == "true"
}

func GetPortOverride() (port int, configured bool, err error) {
	value, ok := os.LookupEnv("ZUI_PORT")
	...
	if err != nil {
		return 0, true, fmt.Errorf("parse ZUI_PORT: %w", err)
	}
	if port < 1 || port > 65535 {
		return 0, true, fmt.Errorf("ZUI_PORT must be between 1 and 65535")
	}

	return port, true, nil
}

// GetBinFolderPath returns the path to the binary folder, defaulting to "bin" if not set via ZUI_BIN_FOLDER.
func GetBinFolderPath() string {
	binFolderPath := os.Getenv("ZUI_BIN_FOLDER")
	if binFolderPath == "" {
		binFolderPath = "bin"
	}
...
func GetDBFolderPath() string {
	dbFolderPath := os.Getenv("ZUI_DB_FOLDER")
	if dbFolderPath != "" {
		return dbFolderPath
	}
...
// ZUI_MAIN_FOLDER, so it survives an update regardless of what happens to
// that folder.
...
func GetDBKind() string {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("ZUI_DB_TYPE")))
	switch v {
...
// GetDBDSN returns the PostgreSQL DSN from ZUI_DB_DSN. Empty for sqlite.
func GetDBDSN() string {
	return strings.TrimSpace(os.Getenv("ZUI_DB_DSN"))
}
...
func GetLogFolder() string {
	logFolderPath := os.Getenv("ZUI_LOG_FOLDER")
	if logFolderPath != "" {
	...
	if testing.Testing() {
		return filepath.Join(os.TempDir(), "z-ui-test-log")
	}
	if runtime.GOOS == "windows" {
...
func init() {
	...
	if os.Getenv("ZUI_DB_FOLDER") != "" {
		return
	}
	oldDBFolder := "/etc/z-ui"
