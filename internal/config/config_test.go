...
func TestGetPortOverride(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if tt.set {
				t.Setenv("ZUI_PORT", tt.value)
			} else {
				original, existed := os.LookupEnv("ZUI_PORT")
				if err := os.Unsetenv("ZUI_PORT"); err != nil {
					t.Fatalf("unset ZUI_PORT: %v", err)
				}
				t.Cleanup(func() {
					if existed {
						_ = os.Setenv("ZUI_PORT", original)
					} else {
						_ = os.Unsetenv("ZUI_PORT")
					}
				})
			}
