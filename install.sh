...
# Postgres env file (/etc/default/z-ui).
write_install_result() {
    local u="$1" p="$2" port="$3" wbp="$4" scheme="$5" host="$6" token="$7" dbtype="$8"
    local result_file="/etc/z-ui/install-result.env"
    local url_host="${host:-SERVER_IP_UNKNOWN}"
    install -d -m 755 /etc/z-ui 2> /dev/null
    local prev_umask
    prev_umask=$(umask)
    umask 077
    if ! {
...
pg_ensure_hba_password_auth() {
    ...
    tmp=$(mktemp) || return 1
    {
        echo "# Added by 3z-ui: allow password logins for the panel database."
...
setup_ip_certificate() {
    ...
    echo -e "${yellow}acme.sh will automatically renew and reload z-ui before expiry.${plain}"
    return 0
}
...
config_after_install() {
    local existing_hasDefaultCredential=$(${xui_folder}/z-ui setting -show true | grep -Eo 'hasDefaultCredential: .+' | awk '{print $2}')
    ...
    local existing_cert=$(${xui_folder}/z-ui setting -getCert true | grep 'cert:' | awk -F': ' '{print $2}' | tr -d '[:space:]')
    local URL_lists=(
    ...
    if [[ ${#existing_webBasePath} -lt 4 ]]; then
        if [[ "$existing_hasDefaultCredential" == "true" ]]; then
            ...
            local db_label="SQLite (/etc/z-ui/z-ui.db)"
            echo ""
            echo -e "${green}═══════════════════════════════════════════${plain}"
            ...
            if [[ "$db_choice" == "2" ]]; then
                ...
                case "${release}" in
                    ubuntu | debian | armbian)
                        xui_env_file="/etc/default/z-ui"
                        ;;
                    arch | manjaro | parch | alpine)
                        xui_env_file="/etc/conf.d/z-ui"
                        ;;
                    *)
                        xui_env_file="/etc/sysconfig/z-ui"
                        ;;
                esac
                ...
                while [[ -z "$xui_dsn" ]]; do
                    if [[ "$NONINTERACTIVE" == "1" ]]; then
                        ...
                        local pg_cred_file
                        pg_cred_file=$(mktemp 2> /dev/null) || pg_cred_file=$(mktemp -t z-ui-pg-creds.XXXXXXXX)
...
EOF
            ...
            ${xui_folder}/z-ui setting -username "${config_username}" -password "${config_password}" -port "${config_port}" -webBasePath "${config_webBasePath}"
            ...
            local config_apiToken=$(${xui_folder}/z-ui setting -getApiToken true | grep -Eo 'apiToken: .+' | awk '{print $2}')
        ...
        if [[ "$existing_hasDefaultCredential" == "true" ]]; then
            ...
            ${xui_folder}/z-ui setting -username "${config_username}" -password "${config_password}"
            ...
            config_apiToken=$(${xui_folder}/z-ui setting -getApiToken true | grep -Eo 'apiToken: .+' | awk '{print $2}')
            : "${SSL_SCHEME:=https}"
            : "${SSL_HOST:=${server_ip}}"
        ...
        # Properly detect empty cert by checking if cert: line exists and has content after it
        existing_cert=$(${xui_folder}/z-ui setting -getCert true | grep 'cert:' | awk -F': ' '{print $2}' | tr -d '[:space:]')
        if [[ -z "$existing_cert" ]]; then
            echo ""
            echo -e "${green}═══════════════════════════════════════════${plain}"
            echo -e "${green}     SSL Certificate Setup (RECOMMENDED)   ${plain}"
...
# by invoking the freshly installed z-ui CLI. IP Limit is load-bearing on
...
install_z-ui() {
    ...
    # Download resources
    if [ $# == 0 ]; then
        tag_version=$(curl -Ls --retry 5 --retry-delay 3 --connect-timeout 15 --max-time 60 "https://api.github.com/repos/MHSanaei/3z-ui/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        if [[ ! -n "$tag_version" ]]; then
            echo -e "${red}Failed to fetch z-ui version, it may be due to GitHub API restrictions, please try it later${plain}"
            exit 1
        fi
        echo -e "Got z-ui latest version: ${tag_version}, beginning the installation..."
        curl -fLR --retry 5 --retry-delay 3 --connect-timeout 15 --max-time 300 -o ${xui_folder}-linux-$(arch).tar.gz https://github.com/MHSanaei/3z-ui/releases/download/${tag_version}/z-ui-linux-$(arch).tar.gz
        if [[ $? -ne 0 ]]; then
        ...
        if [[ ! -s ${xui_folder}-linux-$(arch).tar.gz ]]; then
            ...
            echo -e "${red}Downloaded z-ui release archive is empty${plain}"
            exit 1
        fi
    else
        tag_version=$1
        ...
        url="https://github.com/MHSanaei/3z-ui/releases/download/${tag_version}/z-ui-linux-$(arch).tar.gz"
        echo -e "Beginning to install z-ui ${tag_version}"
        curl -fLR --retry 5 --retry-delay 3 --connect-timeout 15 --max-time 300 -o ${xui_folder}-linux-$(arch).tar.gz ${url}
        if [[ $? -ne 0 ]]; then
            echo -e "${red}Download z-ui ${tag_version} failed, please check if the version exists ${plain}"
            exit 1
        fi
        if [[ ! -s ${xui_folder}-linux-$(arch).tar.gz ]]; then
            rm ${xui_folder}-linux-$(arch).tar.gz -f
    ...
    curl -fLRo "${xui_script_temp}" https://raw.githubusercontent.com/MHSanaei/3z-ui/main/z-ui.sh
    if [[ $? -ne 0 ]]; then
        rm -f "${xui_script_temp}"
        echo -e "${red}Failed to download z-ui.sh${plain}"
        exit 1
    fi
    if [[ ! -s "${xui_script_temp}" ]]; then
        rm -f "${xui_script_temp}"
        echo -e "${red}Downloaded z-ui.sh is empty${plain}"
        exit 1
    fi

    # Stop z-ui service and remove old resources
    if [[ -e ${xui_folder}/ ]]; then
        if [[ $release == "alpine" ]]; then
            rc-service z-ui stop
        else
            systemctl stop z-ui
        fi
    ...
    if [[ $? -ne 0 ]]; then
        ...
        rm -f "${xui_script_temp}"
        echo -e "${red}Failed to extract the z-ui release archive -- the previous installation has already been removed, so the panel will not start until this is fixed; try running the installer again${plain}"
        exit 1
    fi
    rm z-ui-linux-$(arch).tar.gz -f
    ...
    if [[ $? -ne 0 || ! -s z-ui ]]; then
        rm -f "${xui_script_temp}"
        echo -e "${red}Extracted z-ui archive is missing the z-ui binary -- the previous installation has already been removed, so the panel will not start until this is fixed; try running the installer again${plain}"
        exit 1
    fi
    chmod +x z-ui
    chmod +x z-ui.sh
    ...
    # Update z-ui cli and se set permission
    mv -f "${xui_script_temp}" /usr/bin/z-ui
    if [[ $? -ne 0 ]]; then
        rm -f "${xui_script_temp}"
        echo -e "${red}Failed to install z-ui.sh${plain}"
        exit 1
    fi
    chmod +x /usr/bin/z-ui
    mkdir -p /var/log/z-ui
    config_after_install

    # Etckeeper compatibility
    if [ -d "/etc/.git" ]; then
        if [ -f "/etc/.gitignore" ]; then
            if ! grep -q "z-ui/z-ui.db" "/etc/.gitignore"; then
                echo "" >> "/etc/.gitignore"
                echo "z-ui/z-ui.db" >> "/etc/.gitignore"
                echo -e "${green}Added z-ui.db to /etc/.gitignore for etckeeper${plain}"
            fi
        else
            echo "z-ui/z-ui.db" > "/etc/.gitignore"
            echo -e "${green}Created /etc/.gitignore and added z-ui.db for etckeeper${plain}"
        fi
    fi
    ...
    if [[ $release == "alpine" ]]; then
        ...
        rm -f "${xui_rc_temp}"
        curl -fLRo "${xui_rc_temp}" https://raw.githubusercontent.com/MHSanaei/3z-ui/main/z-ui.rc
        if [[ $? -ne 0 ]]; then
            rm -f "${xui_rc_temp}"
            echo -e "${red}Failed to download z-ui.rc${plain}"
            exit 1
        fi
        if [[ ! -s "${xui_rc_temp}" ]]; then
            rm -f "${xui_rc_temp}"
            echo -e "${red}Downloaded z-ui.rc is empty${plain}"
            exit 1
        fi
        mv -f "${xui_rc_temp}" /etc/init.d/z-ui
        if [[ $? -ne 0 ]]; then
            rm -f "${xui_rc_temp}"
            echo -e "${red}Failed to install z-ui.rc${plain}"
            exit 1
        fi
        chmod +x /etc/init.d/z-ui
        rc-update add z-ui
        rc-service z-ui start
    else
        # Install systemd service file
        service_installed=false

        if [ -f "z-ui.service" ]; then
            echo -e "${green}Found z-ui.service in extracted files, installing...${plain}"
            ...
            case "${release}" in
                ubuntu | debian | armbian)
                    if [ -f "z-ui.service.debian" ]; then
                        echo -e "${green}Found z-ui.service.debian in extracted files, installing...${plain}"
                ...
                arch | manjaro | parch)
                    if [ -f "z-ui.service.arch" ]; then
                        echo -e "${green}Found z-ui.service.arch in extracted files, installing...${plain}"
                ...
                *)
                    if [ -f "z-ui.service.rhel" ]; then
                        echo -e "${green}Found z-ui.service.rhel in extracted files, installing...${plain}"
        ...
        if [ "$service_installed" = true ]; then
            ...
            echo -e "${red}Failed to install z-ui.service file${plain}"
            exit 1
        fi
    fi
    ...
    echo -e "${green}z-ui ${tag_version}${plain} installation finished, it is running now..."
    echo -e ""
    echo -e "┌───────────────────────────────────────────────────────┐
