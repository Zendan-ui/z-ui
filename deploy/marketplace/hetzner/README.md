# z-ui on Hetzner Cloud

Hetzner Cloud does **not** have a third-party image marketplace the way AWS does.
Ship z-ui via **cloud-init**: each instance installs non-interactively and
...
z-ui non-interactively and generates unique per-instance credentials.
...
```bash
ssh root@<server-ip> 'cat /etc/z-ui/install-result.env'
```
...
is essentially a documented cloud-init config plus metadata. To propose z-ui as
