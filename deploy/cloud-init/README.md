...
A single [`cloud-init.yaml`](cloud-init.yaml) user-data file that installs z-ui
...
4. The generated credentials are written to `/etc/z-ui/install-result.env`
   (mode 600), echoed to the **serial console**, and appended to `/etc/motd`.

Retrieve them after boot with either:

```bash
sudo cat /etc/z-ui/install-result.env     # over SSH
```
