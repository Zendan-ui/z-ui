// Package main is the entry point for the z-ui web panel application.
...
import (
	...
	"github.com/Zendan-ui/z-ui/v3/internal/database"
	"github.com/Zendan-ui/z-ui/v3/internal/logger"
	"github.com/Zendan-ui/z-ui/v3/internal/sub"
	"github.com/Zendan-ui/z-ui/v3/internal/tunnelmonitor"
	"github.com/Zendan-ui/z-ui/v3/internal/util/crypto"
	"github.com/Zendan-ui/z-ui/v3/internal/util/sys"
	"github.com/Zendan-ui/z-ui/v3/internal/web"
	"github.com/Zendan-ui/z-ui/v3/internal/web/global"
...
// runWebServer initializes and starts the web server for the z-ui panel.
func runWebServer() {
	...
	if monitorCfg.Enabled {
		if monitorCfg.ProxyURL == "" {
			logger.Warning("Tunnel health monitor enabled without ZUI_TUNNEL_HEALTH_PROXY: the probe measures host connectivity, not the xray tunnel, so failures will restart xray without fixing host network issues")
		}
...
// migrateDb performs database migration operations for the z-ui panel.
func migrateDb() {
...
// "z-ui setting" hit the same database backend as the panel. godotenv.Load does
...
// main is the entry point of the z-ui application.
...
func main() {
	...
	migrateDbCmd.StringVar(&migrateSrc, "src", "", "Source SQLite file (defaults to the configured z-ui.db)")
	...
	flag.Usage = func() {
		...
		fmt.Println("    migrate        migrate form other/old z-ui")
