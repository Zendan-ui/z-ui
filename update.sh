...
xui_folder="${XUI_MAIN_FOLDER:=/usr/local/z-ui}"
...
xui_update_status_file="${XUI_UPDATE_STATUS_FILE:-/etc/z-ui/update-status.json}"
...
xui_env_file_path() {
    case "${release}" in
        ubuntu | debian | armbian)
            echo "/etc/default/z-ui"
            ;;
        arch | manjaro | parch | alpine)
            echo "/etc/conf.d/z-ui"
            ;;
        *)
            echo "/etc/sysconfig/z-ui"
            ;;
    esac
}
...
setup_ssl_certificate() {
    ...
    if [ $? -ne 0 ]; then
        ...
        echo -e "${yellow}Please ensure port 80 is open and try again later with: z-ui${plain}"
    ...
    ~/.acme.sh/acme.sh --installcert --force -d ${domain} \
        ...
        --reloadcmd "systemctl restart z-ui" > /dev/null 2>&1
    ...
    if [[ -f "$webCertFile" && -f "$webKeyFile" ]]; then
        ${xui_folder}/z-ui cert -webCert "$webCertFile" -webCertKey "$webKeyFile" > /dev/null 2>&1
...
setup_ip_certificate() {
    ...
    local reloadCmd="systemctl restart z-ui 2>/dev/null || rc-service z-ui restart 2>/dev/null || true"
    ...
    ${xui_folder}/z-ui cert -webCert "${certDir}/fullchain.pem" -webCertKey "${certDir}/privkey.pem"
    if [ $? -ne 0 ]; then
...
ssl_cert_issue() {
    local existing_webBasePath=$(${xui_folder}/z-ui setting -show true | grep 'webBasePath:' | awk -F': ' '{print $2}' | tr -d '[:space:]' | sed 's#^/##')
    local existing_port=$(${xui_folder}/z-ui setting -show true | grep 'port:' | awk -F': ' '{print $2}' | tr -d '[:space:]')
    ...
    systemctl stop z-ui 2> /dev/null || rc-service z-ui stop 2> /dev/null
    ...
    if [[ ${cert_exists} -eq 0 ]]; then
        ...
        if [ $? -ne 0 ]; then
            ...
            systemctl start z-ui 2> /dev/null || rc-service z-ui start 2> /dev/null
            return 1
        else
    ...
    reloadCmd="systemctl restart z-ui || rc-service z-ui restart"
    echo -e "${green}Default --reloadcmd for ACME is: ${yellow}systemctl restart z-ui || rc-service z-ui restart${plain}"
    echo -e "${green}This command will run on every certificate issue and renew.${plain}"
    read -rp "Would you like to modify --reloadcmd for ACME? (y/n): " setReloadcmd
    if [[ "$setReloadcmd" == "y" || "$setReloadcmd" == "Y" ]]; then
        echo -e "\n${green}\t1.${plain} Preset: systemctl reload nginx ; systemctl restart z-ui"
        echo -e "${green}\t2.${plain} Input your own command"
        echo -e "${green}\t0.${plain} Keep default reloadcmd"
        read -rp "Choose an option: " choice
        case "$choice" in
            1)
                echo -e "${green}Reloadcmd is: systemctl reload nginx ; systemctl restart z-ui${plain}"
                reloadCmd="systemctl reload nginx ; systemctl restart z-ui"
                ;;
            2)
                echo -e "${yellow}It's recommended to put z-ui restart at the end${plain}"
    ...
    if [[ "$setPanel" == "y" || "$setPanel" == "Y" ]]; then
        ...
        if [[ -f "$webCertFile" && -f "$webKeyFile" ]]; then
            ${xui_folder}/z-ui cert -webCert "$webCertFile" -webCertKey "$webKeyFile"
            ...
            systemctl restart z-ui 2> /dev/null || rc-service z-ui restart 2> /dev/null
        else
    ...
    case "$ssl_choice" in
        ...
        2)
            ...
            if [[ $release == "alpine" ]]; then
                rc-service z-ui stop > /dev/null 2>&1
            else
            ...
            if [[ $release == "alpine" ]]; then
                rc-service z-ui restart > /dev/null 2>&1
            else
                systemctl restart z-ui > /dev/null 2>&1
            fi
        ...
        3)
            ...
            # 3.4 Apply Settings via z-ui binary
            ${xui_folder}/z-ui cert -webCert "$custom_cert" -webCertKey "$custom_key" > /dev/null 2>&1
            ...
            systemctl restart z-ui > /dev/null 2>&1 || rc-service z-ui restart > /dev/null 2>&1
            ;;
        4)
            echo ""
            ...
            if [[ "$bind_local" == "y" || "$bind_local" == "Y" ]]; then
                ${xui_folder}/z-ui setting -listenIP "127.0.0.1" > /dev/null 2>&1
                SSL_HOST="127.0.0.1"
...
config_after_update() {
    ...
    echo -e "${yellow}z-ui settings:${plain}"
    ${xui_folder}/z-ui setting -show true
    ${xui_folder}/z-ui migrate

    # Properly detect empty cert by checking if cert: line exists and has content after it
    local existing_cert=$(${xui_folder}/z-ui setting -getCert true 2> /dev/null | grep 'cert:' | awk -F': ' '{print $2}' | tr -d '[:space:]')
    local existing_port=$(${xui_folder}/z-ui setting -show true | grep -Eo 'port: .+' | awk '{print $2}')
    local existing_webBasePath=$(${xui_folder}/z-ui setting -show true | grep -Eo 'webBasePath: .+' | awk '{print $2}' | sed 's#^/##')
    ...
    if [[ ${#existing_webBasePath} -lt 4 ]]; then
        ...
        ${xui_folder}/z-ui setting -webBasePath "${config_webBasePath}"
...
# by invoking the freshly downloaded z-ui CLI. IP Limit is load-bearing on
...
setup_fail2ban() {
    ...
    if [[ ! -x /usr/bin/z-ui ]]; then
        echo -e "${yellow}z-ui CLI not found; skipping Fail2ban auto-setup.${plain}"
        return 0
    fi

    echo -e "${green}Setting up Fail2ban for the IP Limit feature...${plain}"
    if /usr/bin/z-ui setup-fail2ban; then
        echo -e "${green}Fail2ban setup complete.${plain}"
    else
        echo -e "${yellow}Fail2ban setup did not finish; IP Limit stays disabled until you run 'z-ui' and open the IP Limit menu. Continuing.${plain}"
    fi
    return 0
}

# Lands a systemd unit file at ${xui_service}/z-ui.service via a temp file +
...
update_z-ui() {
    ...
    if [ -f "${xui_folder}/z-ui" ]; then
        current_xui_version=$(${xui_folder}/z-ui -v)
        echo -e "${green}Current z-ui version: ${current_xui_version}${plain}"
    else
        _fail "ERROR: Current z-ui version: unknown"
    fi

    echo -e "${green}Downloading new z-ui version...${plain}"
    ...
    if [[ -n "${XUI_UPDATE_TAG}" ]]; then
        ...
        tag_version=$(${curl_bin} -Ls "https://api.github.com/repos/MHSanaei/3z-ui/releases/latest" 2> /dev/null | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        if [[ ! -n "$tag_version" ]]; then
            _fail "ERROR: Failed to fetch z-ui version, it may be due to GitHub API restrictions, please try it later"
        fi
    fi
    echo -e "Got z-ui latest version: ${tag_version}, beginning the installation..."
    ${curl_bin} -fLRo ${xui_folder}-linux-$(arch).tar.gz https://github.com/MHSanaei/3z-ui/releases/download/${tag_version}/z-ui-linux-$(arch).tar.gz 2> /dev/null
    if [[ $? -ne 0 ]]; then
        _fail "ERROR: Failed to download z-ui, please be sure that your server can access GitHub"
    fi
    if [[ ! -s ${xui_folder}-linux-$(arch).tar.gz ]]; then
        rm ${xui_folder}-linux-$(arch).tar.gz -f > /dev/null 2>&1
        _fail "ERROR: Downloaded z-ui release archive is empty, please be sure that your server can access GitHub"
    fi

    if [[ -e ${xui_folder}/ ]]; then
        echo -e "${green}Stopping z-ui...${plain}"
        if [[ $release == "alpine" ]]; then
            if [ -f "/etc/init.d/z-ui" ]; then
                rc-service z-ui stop > /dev/null 2>&1
                rc-update del z-ui > /dev/null 2>&1
                echo -e "${green}Removing old service unit version...${plain}"
                rm -f /etc/init.d/z-ui > /dev/null 2>&1
            else
                rm z-ui-linux-$(arch).tar.gz -f > /dev/null 2>&1
                _fail "ERROR: z-ui service unit not installed."
            fi
        else
            if [ -f "${xui_service}/z-ui.service" ]; then
                systemctl stop z-ui > /dev/null 2>&1
                systemctl disable z-ui > /dev/null 2>&1
                echo -e "${green}Removing old systemd unit version...${plain}"
                rm ${xui_service}/z-ui.service -f > /dev/null 2>&1
                systemctl daemon-reload > /dev/null 2>&1
            else
                rm z-ui-linux-$(arch).tar.gz -f > /dev/null 2>&1
                _fail "ERROR: z-ui systemd unit not installed."
            fi
        fi
        # Kill any leftover mtg (MTProto) sidecars. z-ui runs them outside its own
        ...
        echo -e "${green}Removing old z-ui version...${plain}"
        rm ${xui_folder} -f > /dev/null 2>&1
        rm ${xui_folder}/z-ui.service -f > /dev/null 2>&1
        rm ${xui_folder}/z-ui.service.debian -f > /dev/null 2>&1
        rm ${xui_folder}/z-ui.service.arch -f > /dev/null 2>&1
        rm ${xui_folder}/z-ui.service.rhel -f > /dev/null 2>&1
        rm ${xui_folder}/z-ui -f > /dev/null 2>&1
        rm ${xui_folder}/z-ui.sh -f > /dev/null 2>&1
        ...
        rm z-ui-linux-$(arch).tar.gz -f > /dev/null 2>&1
        _fail "ERROR: z-ui not installed."
    fi

    echo -e "${green}Installing new z-ui version...${plain}"
    tar zxvf z-ui-linux-$(arch).tar.gz > /dev/null 2>&1
    if [[ $? -ne 0 ]]; then
        rm z-ui-linux-$(arch).tar.gz -f > /dev/null 2>&1
        _fail "ERROR: Failed to extract the z-ui release archive -- the previous installation has already been removed, so the panel will not start until this is fixed; try running the update again"
    fi
    rm z-ui-linux-$(arch).tar.gz -f > /dev/null 2>&1
    cd z-ui > /dev/null 2>&1
    if [[ $? -ne 0 || ! -s z-ui ]]; then
        _fail "ERROR: Extracted z-ui archive is missing the z-ui binary -- the previous installation has already been removed, so the panel will not start until this is fixed; try running the update again"
    fi
    chmod +x z-ui > /dev/null 2>&1
    ...
    chmod +x z-ui bin/xray-linux-$(arch) > /dev/null 2>&1
    ...
    echo -e "${green}Downloading and installing z-ui.sh script...${plain}"
    local xui_script_temp="/usr/bin/z-ui-temp.$$"
    rm -f "${xui_script_temp}"
    ${curl_bin} -fLRo "${xui_script_temp}" https://raw.githubusercontent.com/MHSanaei/3z-ui/main/z-ui.sh > /dev/null 2>&1
    if [[ $? -ne 0 ]]; then
        rm -f "${xui_script_temp}"
        _fail "ERROR: Failed to download z-ui.sh script, please be sure that your server can access GitHub"
    fi
    if [[ ! -s "${xui_script_temp}" ]]; then
        rm -f "${xui_script_temp}"
        _fail "ERROR: Downloaded z-ui.sh script is empty, please be sure that your server can access GitHub"
    fi
    mv -f "${xui_script_temp}" /usr/bin/z-ui
    if [[ $? -ne 0 ]]; then
        rm -f "${xui_script_temp}"
        _fail "ERROR: Failed to install z-ui.sh script"
    fi

    chmod +x ${xui_folder}/z-ui.sh > /dev/null 2>&1
    chmod +x /usr/bin/z-ui > /dev/null 2>&1
    mkdir -p /var/log/z-ui > /dev/null 2>&1
    ...
    if [[ $release == "alpine" ]]; then
        echo -e "${green}Downloading and installing startup unit z-ui.rc...${plain}"
        xui_rc_temp="/etc/init.d/z-ui.tmp.$$"
        rm -f "${xui_rc_temp}"
        ${curl_bin} -fLRo "${xui_rc_temp}" https://raw.githubusercontent.com/MHSanaei/3z-ui/main/z-ui.rc > /dev/null 2>&1
        if [[ $? -ne 0 ]]; then
            rm -f "${xui_rc_temp}"
            _fail "ERROR: Failed to download startup unit z-ui.rc, please be sure that your server can access GitHub"
        fi
        if [[ ! -s "${xui_rc_temp}" ]]; then
            rm -f "${xui_rc_temp}"
            _fail "ERROR: Downloaded startup unit z-ui.rc is empty, please be sure that your server can access GitHub"
        fi
        mv -f "${xui_rc_temp}" /etc/init.d/z-ui
        if [[ $? -ne 0 ]]; then
            rm -f "${xui_rc_temp}"
            _fail "ERROR: Failed to install startup unit z-ui.rc"
        fi
        chmod +x /etc/init.d/z-ui > /dev/null 2>&1
        chown root:root /etc/init.d/z-ui > /dev/null 2>&1
        rc-update add z-ui > /dev/null 2>&1
        rc-service z-ui start > /dev/null 2>&1
    else
        if [ -f "z-ui.service" ]; then
            echo -e "${green}Installing systemd unit...${plain}"
            if ! _install_xui_service_unit "z-ui.service" "false"; then
                echo -e "${red}Failed to copy z-ui.service${plain}"
                exit 1
            fi
        else
            ...
            case "${release}" in
                ubuntu | debian | armbian)
                    if [ -f "z-ui.service.debian" ]; then
                        echo -e "${green}Installing debian-like systemd unit...${plain}"
                        if _install_xui_service_unit "z-ui.service.debian" "false"; then
                            service_installed=true
                        fi
                    fi
                    ;;
                arch | manjaro | parch)
                    if [ -f "z-ui.service.arch" ]; then
                        echo -e "${green}Installing arch-like systemd unit...${plain}"
                        if _install_xui_service_unit "z-ui.service.arch" "false"; then
                            service_installed=true
                        fi
                    fi
                    ;;
                *)
                    if [ -f "z-ui.service.rhel" ]; then
                        echo -e "${green}Installing rhel-like systemd unit...${plain}"
                        if _install_xui_service_unit "z-ui.service.rhel" "false"; then
                            service_installed=true
                        fi
                    fi
                    ;;
            ...
            if [ "$service_installed" = false ]; then
                ...
                case "${release}" in
                    ubuntu | debian | armbian)
                        service_unit_url="https://raw.githubusercontent.com/MHSanaei/3z-ui/main/z-ui.service.debian"
                        ;;
                    arch | manjaro | parch)
                        service_unit_url="https://raw.githubusercontent.com/MHSanaei/3z-ui/main/z-ui.service.arch"
                        ;;
                    *)
                        service_unit_url="https://raw.githubusercontent.com/MHSanaei/3z-ui/main/z-ui.service.rhel"
                        ;;
                esac

                if ! _install_xui_service_unit "$service_unit_url" "true"; then
                    echo -e "${red}Failed to install z-ui.service from GitHub${plain}"
                    exit 1
                fi
            fi
        fi
        chown root:root ${xui_service}/z-ui.service > /dev/null 2>&1
        chmod 644 ${xui_service}/z-ui.service > /dev/null 2>&1
        systemctl daemon-reload > /dev/null 2>&1
        systemctl enable z-ui > /dev/null 2>&1
        systemctl start z-ui > /dev/null 2>&1
    fi
    ...
    echo -e "${green}z-ui ${tag_version}${plain} updating finished, it is running now..."
    echo -e ""
    echo -e "┌───────────────────────────────────────────────────────┐
│  ${blue}z-ui control menu usages (subcommands):${plain}              │
│                                                       │
│  ${blue}z-ui${plain}              - Admin Management Script          │
│  ${blue}z-ui start${plain}        - Start                            │
│  ${blue}z-ui stop${plain}         - Stop                             │
│  ${blue}z-ui restart${plain}      - Restart                          │
│  ${blue}z-ui status${plain}       - Current Status                   │
│  ${blue}z-ui settings${plain}     - Current Settings                 │
│  ${blue}z-ui enable${plain}       - Enable Autostart on OS Startup   │
│  ${blue}z-ui disable${plain}      - Disable Autostart on OS Startup  │
│  ${blue}z-ui log${plain}          - Check logs                       │
│  ${blue}z-ui banlog${plain}       - Check Fail2ban ban logs          │
│  ${blue}z-ui update${plain}       - Update                           │
│  ${blue}z-ui legacy${plain}       - Legacy version                   │
│  ${blue}z-ui install${plain}      - Install                          │
│  ${blue}z-ui uninstall${plain}    - Uninstall                        │
└───────────────────────────────────────────────────────┘"
}
